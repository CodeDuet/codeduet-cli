...
export async function parseArguments(): Promise<CliArgs> {
  const yargsInstance = yargs(hideBin(process.argv))
    .scriptName('codeduet')
    .usage(
      'Usage: codeduet [options] [command]\n\nCodeDuet - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .command('$0', 'Launch CodeDuet', (yargsInstance) =>
      yargsInstance
        .option('model', {
          alias: 'm',
          type: 'string',
        ...
        .option('yolo', {
          alias: ['y', 'dangerously-skip-permissions'],
          type: 'boolean',
          description:
