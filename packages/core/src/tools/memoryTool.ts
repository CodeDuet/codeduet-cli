  ...
  parametersJsonSchema: {
    ...
    properties: {
      ...
      scope: {
        type: 'string',
        description:
          'Where to save the memory: "global" saves to user-level ~/.qwen/[PROVIDER].md (shared across all projects), "project" saves to current project\'s [PROVIDER].md (project-specific). CodeDuet supports CLAUDE.md, GEMINI.md, CHATGPT.md, GROK.md, and QWEN.md files. If not specified, will prompt user to choose.',
        enum: ['global', 'project'],
      },
    },
    required: ['fact'],
...
- \`scope\` (string, optional): Where to save the memory:
  - "global": Saves to user-level ~/.qwen/[PROVIDER].md (shared across all projects)
  - "project": Saves to current project's [PROVIDER].md (project-specific)
  - CodeDuet supports multiple memory files: CLAUDE.md, GEMINI.md, CHATGPT.md, GROK.md, and QWEN.md
  - If not specified, the tool will ask the user where they want to save the memory.
`;

export const GEMINI_CONFIG_DIR = '.qwen';
export const DEFAULT_CONTEXT_FILENAME = 'QWEN.md';
export const MEMORY_SECTION_HEADER = '## CodeDuet Added Memories';

// Supported memory file types for different AI providers
export const SUPPORTED_MEMORY_FILES = [
  'CLAUDE.md',
  'GEMINI.md', 
  'CHATGPT.md',
  'GROK.md',
...
// It defaults to all supported files but can be overridden by setGeminiMdFilename.
let currentGeminiMdFilename: string | string[] = [...SUPPORTED_MEMORY_FILES];
  ...
  implements ModifiableDeclarativeTool<SaveMemoryParams>
{
  ...
  async execute(
    ...
    if (!params.scope) {
      const errorMessage =
        'Please specify where to save this memory. Use scope parameter: "global" for user-level (~/.qwen/[PROVIDER].md) or "project" for current project (./[PROVIDER].md). Supported files: CLAUDE.md, GEMINI.md, CHATGPT.md, GROK.md, QWEN.md.';
      return {
        llmContent: JSON.stringify({ success: false, error: errorMessage }),
